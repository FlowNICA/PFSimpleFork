...
void SimpleFinder::FindParticles()
{
  ...
  for(int iSecPos=0; iSecPos<nSecPoses; iSecPos++)
  {
    ...
    if(pidPos!=pdg_proton) continue;
    
    OutputContainer track;
    
    track.SetChi2Prim(CalculateChiToPrimaryVertex(trackPos, pidPos));
    track.SetId(trackPos.Id());
    track.SetCharge(+1);
  ...
  for(int iSecNeg=0; iSecNeg<nSecNegs; iSecNeg++)
  {
    ...
    if(pidNeg!=pdg_pionMinus) continue;
    
    OutputContainer track;
    
    track.SetChi2Prim(CalculateChiToPrimaryVertex(trackNeg, pidNeg));
    track.SetId(trackNeg.Id());
    track.SetCharge(-1);
